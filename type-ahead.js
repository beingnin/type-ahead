                        ...
                        _timer = setTimeout(function () {
                            ...
                            if (_params && typeof _params === 'object') {
                                for (var i = 0; i < _params.length; i++) {
                                    if (i === 0) _url += '?';
                                    var regex = new RegExp(`{#${_params[i].name}#}`, 'gi');

                                    if (regex.test(_url)) {

                                        _url = _url.replace(`{#${_params[i].name}#}`, _params[i].value());//for replacing parameters as URI component
                                        break;
                                    }
                                    _url += _params[i].name + '=' + _params[i].value() + '&';
                                }
                                _url = _url.trim().replace(/([?&])+$/, '')//remove trainiling spaces,& and ? from URI
                            }
